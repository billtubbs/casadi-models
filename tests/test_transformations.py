...
def test_connect_nonlinear_systems_in_parallel():
    sys1 = SSModelCTLinearFOSISO(input_name="u1", output_name="y1")
    sys2 = SSModelCTLinearFONoGainSISO(input_name="u2", output_name="y2")
    ...
    assert str(sys_combined) == (
        ...
        "input_names=['sys1_u1', 'sys2_u2'], state_names=['sys1_x', 'sys2_x'], "
        "output_names=['sys1_y1', 'sys2_y2'])"
    )

    # With custom keys
    sys3 = SSModelCTLinearFONoGainSISO(input_name="u3", output_name="y3")
    sys_combined = connect_nonlinear_systems_in_parallel(
    ...
    assert str(sys_combined) == (
        ...
        "name='sys1_sys2_sys3', "
        "input_names=['a_u1', 'b_u2', 'c_u3'], "
        "state_names=['a_x', 'b_x', 'c_x'], "
        "output_names=['a_y1', 'b_y2', 'c_y3'])"
    )
...
def test_connect_nonlinear_systems_in_series():
    ...
    assert str(sys_combined) == (
        ...
        "n=2, nu=1, ny=1, "
        "params={'K': SX(K), 'sys1_T1': SX(T1), 'sys2_T1': SX(T1)}, name='sys1_sys2', "
        "input_names=['u1'], state_names=['sys2_x', 'sys1_x'], "
        "output_names=['y2'])"
    )
    ...
    assert str(sys_combined) == (
        ...
        "input_names=['u1'], state_names=['out_x', 'in_x'], "
        "output_names=['y2'])"
    )
